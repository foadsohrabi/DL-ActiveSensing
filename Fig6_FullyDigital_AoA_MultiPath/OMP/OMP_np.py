...
# phi_1_idx_val = np.random.choice(delta_inv,replace=True ,size=[batch_size_test,L])
...
for ii in range(batch_size_test):
    ...
    ###########debug###################
    # z = np.zeros((delta_inv, 1), dtype=np.complex128)
    # z[phi_1_idx_val[ii]] = np.reshape(np.squeeze(alpha_val[ii]),(-1,L,1))
    # y = A @ z/np.sqrt(L)
...
sio.savemat('OMPtau'+str(tau1)+'.mat', dict(mse_aoa=mse_aoa, \
                                          tau1=tau1))