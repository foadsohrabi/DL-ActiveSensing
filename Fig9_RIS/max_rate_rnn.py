...
'This code generates the results for Proposed active sensing method'


'System information'
tau = 1 # To generate fig 9, we trained different networks with different tau values.
num_ris_elements = 64
num_user = 1
irs_Nh = 8
Rician_factor = 10
...
with tf.name_scope("channel_sensing"):
    ...
    for t in range(tau):
        ...
        real_v0 = tf.reshape(w_her[:,0:num_ris_elements],[batch_size, num_ris_elements])
        imag_v0 = tf.reshape(w_her[:,num_ris_elements:2*num_ris_elements],[batch_size, num_ris_elements])
    ...
    w_her_complex = tf.complex(real_v, imag_v)
    w_her_complex = tf.reshape(w_her_complex, [batch_size, num_ris_elements])
    bf_gain = tf.reduce_sum(tf.multiply(w_her_complex, channel_cascaded_input), 1, keepdims=True)
    received_power = tf.reduce_mean(tf.abs(bf_gain)**2)
####################################################################################
####### Loss Function
loss = -received_power
####### Optimizer
...
with tf.Session() as sess:
    if initial_run == 1:
        ...
        saver.restore(sess, './params/addaptive_reflection_snr_'+str(int(snr_temp))+'_'+str(tau))
    ...
    for epoch in range(n_epochs):
        ...
        print('epoch',epoch,'  loss_test:%2.5f'%-loss_val, '  optimal:%2.5f'%optimal_power_val,
              '  best_test:%2.5f  '%-best_loss, -best_loss/optimal_power_val, '   dB:',10*np.log10(-loss_val),'no_increase:',no_increase)
        # if epoch%10==9: #Every 10 iterations it checks if the validation performace is improved, then saves parameters
        if loss_val < best_loss:
            save_path = saver.save(sess, './params/addaptive_reflection_snr_'+str(int(snr_temp))+'_'+str(tau))
            best_loss = loss_val
            no_increase=0
        else:
