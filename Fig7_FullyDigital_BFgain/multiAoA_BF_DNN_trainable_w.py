...
with tf.name_scope("channel_sensing"):
    ...
    W_uplink_real = tf.get_variable("W_uplink_real", shape=(1,tau,N), trainable=True)
    W_uplink_imag = tf.get_variable("W_uplink_imag", shape=(1,tau,N), trainable=True)
    W_uplink = tf.complex(W_uplink_real,W_uplink_imag)
    W_uplink = W_uplink/tf.norm(W_uplink,axis=-1,keepdims=True)
...
with tf.Session() as sess:
    if initial_run == 1:
        ...
        saver.restore(sess, './param/params_multi_AoA_BF_DNN_trainable_w'+str(snrdBvec[idx_SNR_val])+'_'+str(tau))
    ...
    for epoch in range(n_epochs):
        ...
        if epoch % 10 == 9:  # Every 10 iterations it checks if the validation performace is improved, then saves parameters  
            if loss_val < best_loss:
                save_path = saver.save(sess, './param/params_multi_AoA_BF_DNN_trainable_w'+str(snrdBvec[idx_SNR_val])+'_'+str(tau))
                best_loss = loss_val
